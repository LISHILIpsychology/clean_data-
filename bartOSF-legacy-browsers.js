...
flowScheduler.add(clean_dataRoutineBegin());
flowScheduler.add(clean_dataRoutineEachFrame());
flowScheduler.add(clean_dataRoutineEnd());
...
async function experimentInit() {
  ...
  // Initialize components for Routine "clean_data"
  clean_dataClock = new util.Clock();
...
var clean_dataMaxDurationReached;
var clean_dataMaxDuration;
var clean_dataComponents;
function clean_dataRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'clean_data' ---
    t = 0;
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    // keep track of whether this Routine was forcibly ended
    routineForceEnded = false;
    clean_dataClock.reset();
    routineTimer.reset();
    clean_dataMaxDurationReached = false;
    // update component parameters for each repeat
    psychoJS.experiment.addData('clean_data.started', globalClock.getTime());
    clean_dataMaxDuration = null
    // keep track of which components have finished
    clean_dataComponents = [];
    
    clean_dataComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function clean_dataRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'clean_data' ---
    // get current time
    t = clean_dataClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
    // check for quit (typically the Esc key)
    if (psychoJS.experiment.experimentEnded || psychoJS.eventManager.getKeys({keyList:['escape']}).length > 0) {
...
function clean_dataRoutineEnd(snapshot) {
  return async function () {
    //--- Ending Routine 'clean_data' ---
    clean_dataComponents.forEach( function(thisComponent) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
    });
    psychoJS.experiment.addData('clean_data.stopped', globalClock.getTime());
    // the Routine "clean_data" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
...
async function quitPsychoJS(message, isCompleted) {
  ...
  // ------------------------------------------------------
  // Step 4: Convert cleaned data to CSV
  // ------------------------------------------------------
  let cleanedCSV = [Object.keys(cleanDataObj[0])]
      .concat(cleanDataObj)
      .map(it => Object.values(it).toString())
      .join('\n');
  
  // 保存到全局变量，方便下个 Routine 用
  window.cleanedCSV = cleanedCSV;
  window.cleanedFilename = filename;
  
  console.log("✅ Clean data ready, preview:\n", cleanedCSV.slice(0, 500));
  
  
  // ------------------------------------------------------
  // Send cleaned data to OSF via DataPipe
  // ------------------------------------------------------
  ...
  fetch('https://pipe.jspsych.org/api/data', {
      ...
      body: JSON.stringify({
          experimentID: "izYFomugF9XS",  // 你的 DataPipe Experiment ID
          filename: window.cleanedFilename,
          data: window.cleanedCSV,
      }),
